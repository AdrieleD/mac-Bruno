...
class mac_impl : public mac {
    ...
    long int change_waiting_TX = 1000; //milisecounds/50
    long int numPackEnviados = 0;
    long int numPackConfirmados = 0;
    long int numRetransmissoes = 0;
    long int freq = 0;
    long int lastPctRcv = -1;
    int tempoEspera = 5; // segundos para encerrar a conexao por time out
...
//    }
    ...
    void frequency_hopping_TX(){
        ...
        while ((isChannelBusy(referenceValueChannelBusy) || (!conection_state)) && (appin)){
            if (freq == 2480000000) {
    	        freq = 2405000000;
                start_communication();
            } else {
