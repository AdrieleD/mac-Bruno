...
#define dout d_debug && std::cout
    ...
    /*Esta funçao faz o envio do pacote de comando com sobrecarga */
    void start_communication(){
        char* request = (char*)malloc(sizeof(char)*1);
        char packRequest[256];
        addr0[0] = 0x41;                                                                      // fazer atribuição do endereço via interface
        addr0[1] = 0xe8;
    ...
    /**
     * Envia pacote de configuraçao direto.
     */
    void sendCmdPackage(SendPackage* pack) {
        sendList.push_back(pack);
        //sendPackageNow(pack);     // pode ser implementada como a de enviar ack
    }
    ...
    bool runSending() {
        while (!sendList.empty()) {
            ...
            }while (it != sendList.end() && conection_state);
