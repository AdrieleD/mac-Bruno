...
#define Base_Station true
#define Frequency_hopping false
#define Radio_1 false
...
class mac_impl : public mac {
    ...
    long int resend_waiting = 40; //milisecounds/40
    long int change_waiting = 50; //milisecounds
...
//                waitingAck = false;
    ...
    bool start() {
        ...
        #if Frequency_hopping == true
            //Inicializa a frequencia inicial de transmição de forma aleatoria
            frequency_initial();

                //Frequency hopping para estação base (Round Robin Crescente a partir da frequencia inicial)
            #if Base_Station == true
                freq_hopping = boost::shared_ptr<gr::thread::thread>                               // Usar quando for estação base
                        (new gr::thread::thread(boost::bind(&mac_impl::frequency_hopping_RX, this)));
            //Frequency hopping para Transmissor (Round Robin Crescente a partir da frequencia inicial)

            #else
                 freq_hopping = boost::shared_ptr<gr::thread::thread>                               // Usar quando for nó transmissor
                         (new gr::thread::thread(boost::bind(&mac_impl::frequency_hopping_TX, this)));
            printf("habilitou frequecy tx");
            #endif
        #else
...
//    }
    ...
    int frequency_initial(){
        ...
        if(d_debug){
            printf("frequencia inicial RX %ld\n", freq);
        }
    }
    /**
 * Função que faz a espera para troca de frequencia.
 */
    void waitFreqChangeTime() {
        boost::posix_time::millisec worFreqResend(change_waiting);
        boost::this_thread::sleep(worFreqResend);
    }
    ...
    void frequency_hopping_RX(){
        while(!conection_state){
            ...
            waitFreq = boost::shared_ptr<gr::thread::thread>
                (new gr::thread::thread(boost::bind(&mac_impl::waitFreqChangeTime, this)));
            if(d_debug){
    ...
    void frequency_hopping_TX(){
        while (isChannelBusy(referenceValueChannelBusy) && !conection_state){
            waitFreq = boost::shared_ptr<gr::thread::thread>
                    (new gr::thread::thread(boost::bind(&mac_impl::waitFreqChangeTime, this)));
            if (isChannelBusy(referenceValueChannelBusy) && !conection_state) {
                if (freq == 2480000000) {
                    freq = 2405000000;
                } else {
                    freq = freq + 5000000;
